...
public class Sample2 {
		...
		public static void ex1() {
			...
			System.out.println("공백을 구분자로 사용하여 정수 입력 : ");
			String[] inputs = sc.nextLine().split(" "); 
			// 사용자 입력 한번만 받아서 사용하기 위함(여러 숫자를 한번에) 
			// + 공백을 기준으로 분리 = 분리한 내용은 ArrayList에 담아줌 
			
			for(String s: inputs) {		
				aList.add(Integer.parseInt(s));
			}
			// 사용자가 입력한 값을 공백을 기준으로 분리하여 ArrayList에 담아줌 
		...
		public static void ex2() {
			...
			System.out.println("공백을 구분자로 사용하여 실수 입력 : ");
			String[] inputs = sc.nextLine().split(" "); 
			...
			double average = total / aList.size();
		...
		public static void ex3() {
			...
			for(String s: inputs) {		
				...
				if(!aList.contains(num)) {		// 동일한 객체가 있는지 확인 + 객체의 값도 동일한지도 확인 
					aList.add(num);
				}
				// if(!(aList.indexOf(num) >= 0) {		num의 인덱스(위치)가 0보다 작으면 = num이 배열에 없으면
				//		aList.add(num);
				// }
			}
			System.out.println(aList);
		...
		public static void ex4() {
			...
			for(int i = 0; i < count;) {
				int randNum = rd.nextInt(max - min + 1) + min;	// rd.nextInt(개수) + 시작값;
				if(!aList.contains(randNum)) {
					aList.add(randNum);
					i++; 	// 중복되지 않아서 배열에 값을 추가할때마다 증가 
				}
			}
			System.out.println(aList);
		}
		...
		public static void ex6() {
			...
			while(aList.size() < 10) {
				...
				if((gcStart.get(Calendar.SECOND) + 3) % 60 == gcEnd.get(Calendar.SECOND)) { 	// 동일하면 3초 간격
					aList.add(rd.nextInt(99) + 1);	// 1에서 99사이의 값(시작값이 1이고 99개 생성이니까) 
					gcStart = gcEnd;
					System.out.println(aList);
				}
			}
